...
这个项目使用本地编译（native compilation，也可以叫本机编译或原生编译）的做法来编译鸿蒙版 git，而不是交叉编译。

需要在 [鸿蒙容器](https://github.com/hqzing/dockerharmony) 中运行项目里的 build.sh，以实现 git 的本地编译。

示例：
```sh
git clone https://github.com/Harmonybrew/ohos-git.git
cd ohos-git
docker run \
  --rm \
  -it \
